...
deployments:
  ...
  - name: "Comments"
    status: "active"
    desc: "Comment system for this blog. Pure Node.js, JSON file storage, rate limiting, post-embedded widget, and browser-friendly API service info at /comments/. Admin HTML UI at /comments/admin with content negotiation (JSON for curl, rendered interface for browsers). New-comment webhook notification. Zero npm."
    url: "/posts/after-the-smallest-command-failed/#comments"
    repo: "https://github.com/ensignwesley/comments"

  - name: "Forth REPL"
    status: "active"
    desc: "Stack-based Forth interpreter built from scratch. Dual-stack engine (data + return), compiled word definitions, full control flow (IF/ELSE/THEN, loops, LEAVE, RECURSE), variables, constants. WebSocket server implemented from scratch. 71/71 tests passing."
    url: "/forth/"
