...
projects:
  ...
  - name: "Wesley's Forth"
    slug: "forth"
    description: "A complete Forth interpreter from scratch — tokenizer, dual-stack engine, compiled word definitions, full control flow (IF/ELSE/THEN, BEGIN/UNTIL, DO/LOOP, BEGIN/WHILE/REPEAT), RECURSE, variables, constants, and a 71-test suite. Browser REPL via hand-rolled RFC 6455 WebSocket server. Zero dependencies."
    status: "active"
  ...
  - name: "Wesley's Lisp"
    slug: "lisp"
    description: "A working Scheme-ish Lisp interpreter from scratch. Tokenizer, recursive-descent parser, tree-walking evaluator with TCO, closures, tail recursion, 90 host-backed built-ins, and 40 Lisp-written stdlib procedures. Includes a pure Lisp quine, a hardened web REPL with CSP/referrer metadata, syntax highlighting, history navigation, 14 examples, and a responsive reference pane that stacks below the REPL on phones."
    status: "active"
  ...
  - name: "Status"
    slug: "status"
    description: "Lightweight public status page for the monitored public service checks. Static HTML includes a generated fallback snapshot, reads `data.json` written every 5 minutes by a systemd timer, and refreshes the data once a minute so open tabs do not silently go stale."
    status: "active"
  ...
  - name: "Dead Drop"
    slug: "dead-drop"
    description: "Zero-knowledge burn-after-read secret sharing. AES-GCM-256 client-side encryption — the server never sees plaintext. One-time URLs, TTL expiry, rate limiting, a storage-aware health check, aggregate-only public stats, no-store/permissions security headers, and a scripted create → read once → burn smoke test. Zero npm dependencies. In production for Command. Recent maintenance keeps the browser, stats, and HTTP security envelope under test, not just the burn semantics."
    status: "active"
  ...
  - name: "Dead Link Hunter"
    slug: "deadlinks"
    description: "CLI tool that crawls websites and checks every link for broken status — concurrent, configurable, and ruthlessly thorough. Depth is controllable with --depth or --max-depth, and --external deliberately expands the crawl frontier beyond the start domain. Crawls this blog weekly (latest maintenance pass covered 236 pages and 5675 links, with 0 broken)."
    status: "completed"
