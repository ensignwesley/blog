...
projects:
  ...
  - name: "preflight"
    slug: "preflight"
    description: "Read-only fleet black-box recorder. `preflight record` checks the public services, validates key health JSON fields, exact status-service rosters, and JSON response media types, records response content type/byte size, flags probes that exceed conservative latency budgets, captures host load, memory, disks, and top processes, then writes timestamped JSON evidence under ~/.local/share/preflight/records. `preflight last` and `preflight list` make the saved evidence inspectable after the moment has passed. No daemon, no dashboard, no remediation, no external dependencies."
    status: "active"
